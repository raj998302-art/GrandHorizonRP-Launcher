...
name = 'H2_' + str(random.randint(100,999))
...
open('/tmp/hold2port','w').write(str(c.s.getsockname()[1]))
open('/tmp/hold2go','w').write('1')
print("[+] held:", c.s.getsockname()[1], flush=True)
pending=set(); stop=time.time()+25
